...
st.set_page_config(page_title="SABA IMPERIAL | Ultimate Gamified AI Platform", page_icon="⚡", layout="wide")
...
section = st.sidebar.selectbox("اختر ركن التعلم:", [
    "🎮 لوحة التلعيب والمستويات (Gamification)",
    "⏱️ دروس سريعة (Micro-Lessons)",
    "🎯 المهارات الأربع المتكاملة",
    "🤖 التخصيص الذكي (Duolingo Max Style)",
...
if section == "🎮 لوحة التلعيب والمستويات (Gamification)":
    st.markdown("<div class='feature-card'><h1>🎮 نظام التلعيب الإمبراطوري</h1><p>تصدّر قائمة المتصدرين، اجمع النقاط، وافتح تحديات جديدة مع كل إنجاز لغوي.</p></div>", unsafe_allow_html=True)
...
elif section == "⏱️ دروس سريعة (Micro-Lessons)":
    ...
    st.info("📌 درس اليوم السريع: **استراتيجيات الرد السريع في اجتماعات العمل.**")
    ...
    st.markdown("<div class='feature-card'><h1>🎯 التدريب الشامل للمهارات الأربع</h1><p>تمارين مخصصة لتطوير القراءة، الكتابة، الاستماع، والتحدث بكفاءة عالية.</p></div>", unsafe_allow_html=True)
...
elif section == "🤖 التخصيص الذكي (Duolingo Max Style)":
    st.markdown("<div class='feature-card'><h1>🤖 الذكاء الاصطناعي وتقمص الأدوار (Duolingo Max)</h1><p>خوارزميات متقدمة تكييف الدروس حسب مستواك وتتيح لك محادثات حية وتقمص أدوار واقعية.</p></div>", unsafe_allow_html=True)
    ...
    if st.button("🚀 ابدأ جلسة المحادثة الذكية"):
        st.success(f"🎙️ تم تفعيل نموذج المحادثة لسيناريو: ({role_choice}). ابدأ بالتحدث الآن عبر الميكروفون المرتبط بالذكاء الاصطناعي.")

elif section == "🌍 المرونة وإمكانية الوصول":
